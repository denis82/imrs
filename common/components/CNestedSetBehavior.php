...
/**
 * Шаблон отображения имени объекта
 * @var string 
 */
public $templateLabel = "{name}";

/**
 * Метод получения представления объекта
 * @return type
 */
public function getLabel() {
    return preg_replace_callback("/{(\w+)}/", array($this, 'renderLabel'), $this->templateLabel);
}

/**
 * Приминение шаблона представления к объекту
 * @param type $matches
 * @return type
 */
protected function renderLabel($matches) {
    if (key_exists($matches[1], $this->owner->attributes)) {
        return $this->owner->{$matches[1]};
    }
    return $matches[0];
}
