...
class AuditCommand extends CConsoleCommand {
    ...
    public function actionIndex() {
        ...
        foreach ($modelProject as $project) {
                ...
                if($project->status){
                    ...
                    $createFileId = $project->id;
                }

        }
        
        
        $modelProject = Project::model()->findByPk($createFileId);
        $post->status = 0;
        $post->save();
...
//         }
        ...
        $modelProject = Project::model()->findByPk(129);
        echo $modelProject->status;
          $modelProject->status = 1;
          $modelProject->save();
          echo $modelProject->status;
        ...
        $modelProject = Project::model()->findAll();
        foreach ($modelProject as $proj) {

                if(self::PROCESS == $proj->status ){
        ...
        $modelProjectProc = Project::model()->findByPk($id);
        $modelProjectProc->status = self::CRONPROCESS;
        $modelProjectProc->save();
...
//         Yii::app()->end();
        ...
        **/
        ...
        $xml = $this->renderFile(Yii::getPathOfAlias('application.views.report.docx') . '.php', $xml_data, true);
 ...
 //var_dump($dir . $report_name . '.docx');
         //        readfile( $dir . $report_name . '.docx');
 
         $modelProjectDone = Project::model()->findByPk($id);
         $modelProjectDone->status = self::DONE;
         $modelProjectDone->fileName = $report_name;
         $modelProjectDone->save();
         Yii::app()->end();
    }
