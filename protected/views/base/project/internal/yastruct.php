...
<div class="panel panel-flat">
    <div class="panel-heading">
        <h5 class="panel-title text-semiold">Структура сайта по Яндексу</h5>

                <div class="heading-elements">
                    <span class="heading-text jLastUpdate">
                    	<?= $check->date ? TxtHelper::DateTimeFormat( $check->date ) : '' ?>
                    </span>
	...
	foreach ($pages as $el) {
		...
		for ($j = 0; $j < count($url['path']); $j++) {
			if (isset($current[$j+1]) and $current[$j+1] == $url['path'][$j]) {
				...
				print str_pad('', $level, "\t") . '<li data-level="' . $level . '"><span data-href="' . implode('/', array_slice($current, 0, -1)) . '" data-status="200" data-title="' . $el->title . '">/' . $url['path'][$j] . '</span><ul>' . $eol;
			}
		}
	}
...
elseif ($check and count($pages) == 0): 

	?>
		<div class="alert alert-danger alert-styled-left alert-bordered">
			Страниц в Яндексе не найдено.
		</div>
	<?
