	...
	foreach ($data as $el) {
		...
		for ($j = 0; $j < count($url['path']); $j++) {
			if (isset($current[$j+1]) and $current[$j+1] == $url['path'][$j]) {
				...
				print str_pad('', $level, "\t") . '<li data-level="' . $level . '"><span data-href="' . implode('/', array_slice($current, 0, -1)) . '" data-status="' . $el->status . '" data-title="' . $el->title . '">/' . $url['path'][$j] . '</span><ul>' . $eol;
			}
		}
	}
...
elseif ($load_result === false): 

	?>
		<div class="alert alert-danger alert-styled-left alert-bordered">
			Файл sitemap не найден! <?= ($error_code ? 'Ошибка ' . $error_code . '.' : '') ?> Структуру сайта по sitemap сформировать не удалось.
		</div>
	<?
